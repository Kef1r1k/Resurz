...
var Message = function Message(_ref) {
  ...
  var _useState = (0,react.useState)(false),
      ...
      setShowHint = _useState2[1]; // Если text не определен, не рендерим сообщение
  ...
  return /*#__PURE__*/react.createElement("div", {
    ...
    className: "A_HintBubble"
  }, question.hint)), isLastMessage && !isFirstMessage && /*#__PURE__*/react.createElement("button", {
...
var questions = [{
  ...
  type: 'number',
  scenario: ['небольшой заказ', 'заказ с этапами работы'] // Подсказка отсутствует
...
var Chat = function Chat() {
  ...
  var _useState3 = (0,react.useState)([{
    ...
    questionId: questions[0].id
      ...
      setMessages = _useState4[1]; // Сообщения (вопросы и ответы)


  var _useState5 = (0,react.useState)(''),
      _useState6 = Chat_slicedToArray(_useState5, 2),
      userInput = _useState6[0],
      setUserInput = _useState6[1]; // Текущий ввод пользователя


  var _useState7 = (0,react.useState)([]),
      _useState8 = Chat_slicedToArray(_useState7, 2),
      userAnswers = _useState8[0],
      setUserAnswers = _useState8[1]; // Ответы пользователя


  var _useState9 = (0,react.useState)('start'),
      _useState10 = Chat_slicedToArray(_useState9, 2),
      currentScenario = _useState10[0],
      setCurrentScenario = _useState10[1]; // Текущий сценарий


  var _useState11 = (0,react.useState)(false),
      _useState12 = Chat_slicedToArray(_useState11, 2),
      isChatFinished = _useState12[0],
      setIsChatFinished = _useState12[1]; // Флаг завершения чата


  var _useState13 = (0,react.useState)(false),
      _useState14 = Chat_slicedToArray(_useState13, 2),
      isExtendedMode = _useState14[0],
      setIsExtendedMode = _useState14[1]; // Режим расширенных настроек
  // Разделяем вопросы на обычные и расширенные


  var regularQuestions = questions.filter(function (q) {
    return !q.scenario.includes('расширенные настройки');
  });
  var extendedQuestions = questions.filter(function (q) {
    return q.scenario.includes('расширенные настройки');
  }); // Функция для подсчета обычных вопросов

  var getBasicQuestionsCount = function getBasicQuestionsCount(scenario) {
    return questions.filter(function (q) {
      return q.scenario.includes(scenario) && !q.scenario.includes('расширенные настройки');
    }).length;
  }; // Функция для подсчета расширенных вопросов


  var getExtendedQuestionsCount = function getExtendedQuestionsCount(scenario) {
    return questions.filter(function (q) {
      return q.scenario.includes(scenario) && q.scenario.includes('расширенные настройки');
    }).length;
  ...
  var calculateProgress = function calculateProgress() {
    var basicQuestionsCount = getBasicQuestionsCount(currentScenario);
    var extendedQuestionsCount = getExtendedQuestionsCount(currentScenario); // Если выбран режим расширенных настроек, учитываем все вопросы

    if (isExtendedMode) {
      var totalQuestions = basicQuestionsCount + extendedQuestionsCount;
      var currentProgress = currentQuestionIndex + 1; // Текущий прогресс
  ...
  var progress = calculateProgress(); // Обработка ответа пользователя

  var handleAnswer = function handleAnswer(answer) {
    var currentQuestion = questions[currentQuestionIndex]; // Добавляем ответ пользователя в список ответов
    ...
    var nextScenario = currentScenario;
    ...
    var nextQuestion = questions.find(function (q, index) {
      return index > currentQuestionIndex && q.scenario.includes(nextScenario);
    });

    if (nextQuestion) {
      setCurrentQuestionIndex(questions.indexOf(nextQuestion));
      setCurrentScenario(nextScenario); // Добавляем следующий вопрос от бота в список сообщений
      ...
      setMessages(function (prevMessages) {
        return [].concat(_toConsumableArray(prevMessages), [{
          text: 'Спасибо за ответы! Твой договор готов.',
          isUser: false
        }]);
      });
      console.log('Ответы пользователя:', userAnswers);
  ...
  var handleBack = function handleBack() {
    if (messages.length > 1) {
      ...
      var newMessages = messages.slice(0, -2); // Удаляем два последних сообщения

      setMessages(newMessages); // Обновляем индекс текущего вопроса
  ...
  return /*#__PURE__*/react.createElement("div", {
    ...
    type: (_questions$currentQue = questions[currentQuestionIndex]) === null || _questions$currentQue === void 0 ? void 0 : _questions$currentQue.type // Тип ввода (text/number)
    ,
    value: userInput,
    ...
    disabled: ((_questions$currentQue2 = questions[currentQuestionIndex]) === null || _questions$currentQue2 === void 0 ? void 0 : _questions$currentQue2.type) !== 'text' && ((_questions$currentQue3 = questions[currentQuestionIndex]) === null || _questions$currentQue3 === void 0 ? void 0 : _questions$currentQue3.type) !== 'number'
  })));
};
